# 原创性与文件指纹声明

本示例的全部题面、选项、答案、评分点、诗文、讲义、知识卡和说明文字均为本公开项目从零创作。人物、学校与数据均为虚构。全部文件未复制或改写第三方试题、课文、答案、教师材料、真实机构数据或网络引语。

## 逐文件来源说明

- `README.md`：为本公开项目从零编写的运行与跨智能体说明。
- `source/原创微型示范卷.md`：六板块题面、所有选项、诗文、人物、学校、数据及作文材料均从零创作并显式虚构。
- `source/原创答案与评分参考.md`：全部答案、分值与允许边界从零设计，只是本项目原创示例评分参考。
- `content/00_整卷总览与讲评建议.md`：依据本原创卷从零建立能力地图与课堂安排。
- `content/01_阅读一_信息类文本详案.md`：只分析本卷虚构阅读角材料，不引用现实调查。
- `content/02_阅读二_文学类文本详案.md`：只分析原创虚构短篇《灯亮之前》。
- `content/03_阅读三_文言文详案.md`：只分析原创拟古短篇《周生治圃记》，不冒充古籍。
- `content/04_阅读四_古诗文详案.md`：只分析原创诗《晚课归窗下作》；默写句不是传统名句。
- `content/05_语言文字运用详案.md`：语段、错句、选项和修改过程均从零创作。
- `content/06_作文审题与写作指导详案.md`：材料、结构、虚构论证情境和示范段均从零创作；虚构情境未作为真实事实素材。
- `knowledge/cards/overviews/OV-0001.md`：仅从本原创示例抽取的候选总览卡。
- `knowledge/cards/question-types/QT-0001.md`：仅从本原创第2题抽取的候选题型卡。
- `knowledge/cards/methods/MT-0001.md`：仅从本原创讲义抽取的候选方法卡。
- `knowledge/index.json`：由上述三张候选卡确定性生成，不含公共库或私有库内容。

## SHA-256 清单

清单覆盖 README、source、content、示例知识卡与示例索引；本文件自身为避免自指不纳入哈希。每次修改任一被覆盖文件，都必须重新计算并更新相应值。

- `e7ce2eb17c682b0cb2a0a076b01e1ca39c576e1c378112f93303b92463a34468`  `README.md`
- `5d0355b395610f88a9765be177feab33a814bdb0eda4d9e383e418ff6b924955`  `content/00_整卷总览与讲评建议.md`
- `4f060a68dc777971598f2b74483095d521babe77a93c79d8f12bcf4768bc8db1`  `content/01_阅读一_信息类文本详案.md`
- `1c0aa9a5f7bce27450df3bd437f0db5b788e2979e057e0a3e17e6649a22ec65e`  `content/02_阅读二_文学类文本详案.md`
- `dffdfb159fa9d258e4af2a56f7d5176dfae5c27217225309927df2b1bbdb84df`  `content/03_阅读三_文言文详案.md`
- `d47ea4ce0390286c85ca71025d4a2e85b485129ce16ad46956d9073dc6a3eb64`  `content/04_阅读四_古诗文详案.md`
- `dc108ea3ab548b0bb7ee35f41107472d18793e06d9ceda282d3e24a7bd414ce7`  `content/05_语言文字运用详案.md`
- `3ac5eecc8a66b1072b15eec0282937f5c387af3d0bfe18ca2485bdac49887c04`  `content/06_作文审题与写作指导详案.md`
- `d9c7b2794f1af91bbb5fc5712b73a77c28dbd2bd68613a4c6dd0480cec07d345`  `knowledge/cards/methods/MT-0001.md`
- `ac421b1f2a5d880c57a484c1720c03a3bb49bc60eb843c8745db94e554a623ae`  `knowledge/cards/overviews/OV-0001.md`
- `257717e5bd327e10872547b49f364923258643e0c260405fbd372204a3ca050c`  `knowledge/cards/question-types/QT-0001.md`
- `2aabb7f49a762e99cbdaab1e9660dd62a86886de60f26750c36a8db23eb4b3fb`  `knowledge/index.json`
- `d270759c0b89827b9267f3b3cd932023d6343d0d75f37f10d8e3430f659d2d21`  `source/原创微型示范卷.md`
- `f19876beceece18546805db311d7cea8f46c4a3fe07cb479de0b1fb4e96ec82b`  `source/原创答案与评分参考.md`
